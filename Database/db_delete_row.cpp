...
Delete_Row_Info::Delete_Row_Info(const QString& table_name, const QString& field_name, const std::vector<Delete_Row_Info>& childs, bool set_null) :
    table_name_(table_name), field_name_(field_name), childs_(childs), set_null_(set_null) {}

// ------------------------------------------------------------------------------------------------------------------

Delete_Row_Helper::Delete_Row_Helper(Base *obj, const QString& id) : obj_(obj), id_(id) {}

bool Delete_Row_Helper::del(const QString &table_name, const std::vector<Delete_Row_Info>& delete_rows_info)
{
    try
    {
        QString where;
        ...
        if (!obj_->del(table_name, "id=" + id_).isActive())
...
void Delete_Row_Helper::del_impl(const Delete_Row_Info& del_parent)
{
    ...
    for (const Delete_Row_Info& del: del_parent.childs_)
    {
        ...
        if (del.set_null_ && del.table_name_ == del_parent.table_name_)
            ...
            where = del.field_name_ + QString(" IN(SELECT id FROM %1 WHERE %2 = %3)")
                    .arg(del_parent.table_name_).arg(del_parent.field_name_).arg(id_);
