...
public:
    ...
    explicit File(const std::string& name, int open_mode, int file_attibute = DEFAULT_CREATE_PERM);
    ~File();
    ...
    static bool write(const std::string& name, const char* data, std::size_t data_size,
                      int open_mode = CREATE | WRITE_ONLY | TRUNCATE, int file_attibute = DEFAULT_CREATE_PERM);
